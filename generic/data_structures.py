...
# I had worked with bits in an earlier project right?
# for hashing.. Let's look at that again	
class BinaryTree:
	def __init__(self):
		self.l_child = None
		self.r_child = None
	...
	def traverse(self, path=""):
		if isinstance(self.l_child, BinaryTree):
			for value, sub_path in self.l_child.traverse(path=path + '0'):
				yield value, sub_path
		else:
			yield self.l_child, path + '0'
		
		if isinstance(self.r_child, BinaryTree):
			for value, sub_path in self.r_child.traverse(path=path + '1'):
				yield value, sub_path
		else:
			yield self.r_child, path + '1'
